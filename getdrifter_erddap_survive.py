...
it used for getting drifter survive information from erddap based on id_list and time_period (only for 1 list of drifters)
...
#id=['55291', '    55292', '    55293', '    57202', '    65204', '    65205', '    65206',]
...
if id==[]:
    ...
    for q in range(len(id)):
        ...
        for z in range(len(lat_data)):
            if haversine(lon_data[z], lat_data[z], lon[-1], lat[-1])< 0.6: #1.0 represent  1 kilometer   , get rid of landed drifter
                print haversine(lon_data[z], lat_data[z], lon[-1], lat[-1])
                distance=1
                break
        if distance==0:
            every_time_total.append(date2num(time[-1])-date2num(time[0]))
            totalday=totalday+date2num(time[-1])-date2num(time[0])  # get number of total ploted drifters
    drifter_n,index_day=[],[]    
    ...
    df=DataFrame(np.array(drifter_n),index=index_day,columns=['Rachel survival']) 
    ax=df.plot(kind='bar',ylim=[0,50])  #plot graph
    ax.text(0.5, 0.8, 'average '+str(round(totalday/(q+1),1))+' days',
        verticalalignment='top', horizontalalignment='center',
        transform=ax.transAxes,
        color='green', fontsize=38)
    ax.set_xlabel('days_period',fontsize=40)
    ax.set_ylabel('drifter amount',fontsize=40)
